...
exports.commands = {
	givedev: function (target, room, user) {
		...
		this.privateModCommand("(" + user.name + " has given Developer to " + targetUser.name + ")");
		saveVips();
	},
	...
	takedev: function (target, room, user) {
		...
		if (!Users.vips[toId(target)]) return this.sendReply("User \"" + target + "\" is not a Developer.");

		delete Users.vips[toId(target)];
		saveVips();
		this.privateModCommand("(" + user.name + " has removed Developer from " + target + ")");
	},
};
